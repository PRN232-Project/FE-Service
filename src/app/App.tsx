...
import { ExamOfficePortal } from './pages/ExamOfficePortal';
...
export interface AuthUser {
  ...
  name: string;
  email: string;
  role: 'ExamOfficer' | 'Lecturer' | 'Admin';
  studentCode?: string;
}
...
export default function App() {
  ...
  if (!currentUser) {
    return <LoginScreen onLogin={(user) => {
      ...
      else if (user.role === 'ExamOfficer') navigate('/exam-office/students');
      else if (user.role === 'Lecturer') navigate('/lecturer/batches');
    }} />;
  }
  ...
  return (
    <div className="h-screen w-full font-sans flex flex-col bg-white overflow-hidden text-gray-900">
      ...
      <div className="flex-1 overflow-hidden">
        <Routes>
          <Route path="/admin/*" element={currentUser.role === 'Admin' ? <AdminPortal currentUser={currentUser} /> : <Navigate to="/" replace />} />
          <Route path="/exam-office/*" element={currentUser.role === 'ExamOfficer' ? <ExamOfficePortal currentUser={currentUser} /> : <Navigate to="/" replace />} />
          <Route path="/lecturer/*" element={currentUser.role === 'Lecturer' ? <LecturerPortal currentUser={currentUser} /> : <Navigate to="/" replace />} />
          <Route path="*" element={<Navigate to={
            currentUser.role === 'Admin' ? '/admin/accounts' : 
            currentUser.role === 'ExamOfficer' ? '/exam-office/students' : 
            '/lecturer/batches'
          } replace />} />
        </Routes>
      </div>
...
function LoginScreen({ onLogin }: { onLogin: (user: AuthUser) => void }) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === 'Student') role = 'ExamOfficer'; // Just for safety if backend hasn't updated yet
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 lg:p-24 bg-white">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <Input 
              label="User ID" 
              placeholder="e.g. admin, lecturer1, examofficer1" 
              value={loginId}
          ...
          <div className="mt-10 pt-6 border-t border-gray-100">
            ...
            <div className="flex flex-wrap gap-2 justify-center lg:text-left text-xs">
              ...
              <span className="px-3 py-1.5 bg-slate-50 border border-slate-200 text-slate-700 rounded-lg"><b>examofficer1</b> <span className="text-slate-400">Exam Office</span></span>
              <span className="px-3 py-1.5 bg-slate-50 border border-slate-200 text-slate-700 rounded-lg"><b>lecturer1</b> <span className="text-slate-400">Lecturer</span></span>
            </div>
          </div>
        </div>
      </div>
